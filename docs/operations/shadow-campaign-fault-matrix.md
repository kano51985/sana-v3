...
| worker health probe 超时/进程泄漏 | Compose structural test；post-Campaign auditor | Celery inspect 探针已替换为有界 direct-exec Redis PING；运行超过多轮 health interval 后仍只有 main+2 prefork 共 3 个进程 | PASS：无 orphan probe；Campaign/queue 审计另证实际消费能力 |
