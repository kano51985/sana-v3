...
7. **候选身份必须不可变。** Commit SHA、Docker image ID、迁移 head、运行时安全配置和评测策略共同定义候选；任一项无法验证时禁止发起 live Campaign。
...
Conversation 创建 API 增加可选 `Idempotency-Key`，数据库使用 `(tenant_id, user_id, creation_idempotency_key)` 唯一约束并保存规范化 title 的 request hash；普通客户端不提供该 Header 时保持现有行为。相同 key 与相同 request hash 返回原 Conversation，相同 key 配不同 title 则返回 409。Runner 为每个调度单元创建独立 Conversation，取得 ID 后先把它写入 Result，再提交 Message。这样可同时避免跨 case 上下文污染和崩溃窗口重复计费：
...
Message API 的幂等重放也必须校验规范化 content hash；同一个 `(tenant, conversation, key)` 配不同内容时返回 409，而不是静默返回旧 receipt。Result 保存提交内容 hash，Collector 再与权威 Message 计算值核对，但不保存原文。
...
Citation 安全指标采用 fail-closed 语义：分母是最终答案中的全部 `FACTUAL` Claim，而不是只有 `GROUNDED/VERIFIED/CONFLICTED` 的 Claim。每个 factual Claim 必须至少有一条 Citation，且 Citation 必须能追踪到属于同一 tenant/run 的 VerifiedEvidence、DocumentVersion、DocumentChunk 和合法 quote offsets。缺少 `claim_kind`、Fact 绑定或链路记录均视为违规；没有 factual Claim 时指标为 `NOT_APPLICABLE`，不能伪装成 100%。
...
动态事实不能由候选模型自评。独立 review 子命令按 GatePolicy 确定性抽取 20 个不同的 answerable case/run，调用正式 API 读取原 Message、回答和 Citation 供人工按版本化 rubric 检查，只把结构化 verdict、严重度和 reason code 写入数据库，不复制 prompt、答案、quote 或网页正文。Reviewer 必须经过同一 tenant 的认证与授权；报告只公开聚合值，不公开 reviewer 身份。
...
- source commit SHA、`source_dirty=false` 证明、API/Worker/Dispatcher 的 Docker image ID、OCI revision label 和 Alembic head。
...
- Profile 锁定的 max runs、max concurrency、estimated-cost stop threshold、Provider-call 调度停止阈值、绝对 Provider-call 上限和在途 reserve。
- planned、submitted、completed、failed、degraded 数量。
- observed token、estimated cost 和 possibly-billed 数量。
- stop reason、created/started/review-deadline/completed time、active wall-clock 和乐观锁 version。
...
- case ID、repetition、稳定 `schedule_ordinal`、locale、category 和 expected mode。
- scheduling state：`PENDING/CLAIMED/CONVERSATION_BOUND/SUBMITTED/COLLECTED/FAILED`。
...
- stable reason codes、reviewer principal reference、reviewed time。

唯一约束为 `(result_id, rubric_version)`。表启用并 FORCE RLS。不得保存 reviewer 的自由文本、prompt、答案、quote、网页正文或模型输出；reviewer 身份只用于内部审计，不进入公开报告。
...
- `answer_claims` 增加 nullable `claim_kind` 与 `fact_requirement_id`。新 Run 写入时两者必须符合领域不变量；历史行可以保持 NULL，但 Campaign Collector 遇到 NULL 必须 fail closed。
...
- `docker-smoke-v1`：只运行 manifest 中 6 个 `smoke=true` case，每个 1 次；max runs 6、max concurrency 2、Provider-call 调度停止阈值 32、绝对调用上限 48、estimated-cost stop threshold 0.01 美元，使用 `shadow-smoke-gate-v1`。
- `shadow-full-v1`：40 case × 3；max runs 120、max concurrency 2、Provider-call 调度停止阈值 480、绝对调用上限 960、estimated-cost stop threshold 0.10 美元，使用 `shadow-gate-v2`。

Profile 的 canonical snapshot/hash 写入 Campaign。改变费用或调用阈值必须发布新 profile version 并取得相应成本授权；不能通过 CLI 临时修改。CLI 和数据库均校验绝对上限不高于 `max_runs × RESEARCH.max_llm_calls`，也不允许把调度停止阈值标记为硬上限。
...
Full profile 的 0.10 美元是平台基于版本化费率、已知 token 和 possibly-billed reserve 执行的**调度停止阈值**，不是对外部 Provider 账单的绝对保证。Provider 不提供平台可依赖的逐 Campaign 预授权；崩溃窗口和延迟 usage 可能令实际账单略高。真正可证明的代码侧硬边界是 120 Run、每 Run 4/8 次调用，因此最坏路由下绝对上限为 960 次；Provider 账户侧额度是最后一道外部边界。报告必须明确区分 observed、reserved、stop threshold 和 absolute ceiling。Smoke profile 同理使用自身的 0.01/32/48 边界。

Runner 在每次 claim/admission 时锁定 Campaign 行，原子检查并更新 remaining Run、observed Provider call、in-flight call reserve 和估算费用 reserve。每个在途 Run 按 RESEARCH 最坏值预留 8 次调用及 GatePolicy 的 token/cost reserve；Result 收集后用真实值结算。POSSIBLY_BILLED 按保守 reserve 计费。达到调度停止阈值后不再提交新 Run，但继续收集已提交 Run。该设计保证最多两个在途 Run 的暴露被显式计入，不宣称 Worker 出站后还能被 Runner 精确中止。
...
`shadow-full-v1` create 还必须引用一个 24 小时内完成且 PASS 的 smoke Campaign。两者的 commit/image、manifest、模型/Worker 配置、collector/safety-invariant version、费率和迁移 head 必须完全相同；SmokeGatePolicy 与 FullGatePolicy 按 profile 分别锁定，不要求名称相同。其他任何候选身份差异都令 smoke 失效并要求重跑。
...
没有 fatal safety violation 且自动样本充分时，Campaign 在结果收集完成后进入 `AWAITING_REVIEW`，review deadline 为 48 小时。`review` 使用确定性 SHA-256 排序，从四个 expected-mode/locale 分层各选择 5 个不同的 answerable case，每个 case 只选一个 repetition，共 20 个 review unit。命令通过认证 API 临时显示 prompt、答案和 Citation，写库内容仅限结构化评分。全部必需 review 完成后才能产生最终 PASS/FAIL；超过 deadline 仍未完成则封存为 INSUFFICIENT_SAMPLE，不能追加迟到 review 改写历史结果。
...
首版 live Campaign 只支持本机 Docker Compose 部署。Runner 在任何可能创建 Conversation、Message 或 Provider 调用的 API 请求前必须：

1. 确认用于 build 的 git worktree 对 tracked runtime 文件为 clean；untracked runtime 文件也必须为空。
2. 读取 API、Worker、Dispatcher 容器的 immutable image ID，验证三者相同，并验证 OCI `org.opencontainers.image.revision` 等于记录的 commit SHA。
3. 通过只读数据库连接确认 Alembic 单一 head 与预期 revision 一致。
4. 对 SearchPolicy、模型角色配置、thinking/output format、prompt/parser schema、Discovery provider registry 和非敏感 Worker 配置生成 allowlist fingerprint。
5. 确认队列中没有其他测试 Run，并记录 Worker concurrency、机器/容器资源上限和 Campaign 开始时队列深度。

任一步无法证明时，在产生 Provider 调用前失败。不得从当前含未提交 runtime 改动的工作区直接 build 正式 Campaign 镜像；应从独立 clean worktree 构建并以 image ID 运行。
...
- `RUNNING/STOPPING -> COMPLETED`：fatal safety 已封账，或受控停止后已收集全部在途 Run 且无需人工 review。
- `RUNNING -> AWAITING_REVIEW -> COMPLETED`：自动样本充分且无 fatal；必需 review 完成后封存质量 gate，或 48 小时 deadline 到达后以 INSUFFICIENT_SAMPLE 封存。
- `CREATED/RUNNING/STOPPING -> ABORTED`：永久配置、provenance 损坏或不可恢复的 Campaign 自身故障；ABORTED 不自动等于候选 FAIL。
...
- 达到 Run、Provider-call 调度停止阈值或 estimated-cost 调度停止阈值。
- 用户中断或显式 stop 请求。
...
- 每个 Run 未超过自身 mode hard deadline，smoke Campaign 没有触发费用或调用停止阈值。
...
- 人工 review 中 `CORRECT` 比例 ≥ 90%，Citation relevance 和 source appropriateness 合格率均 ≥ 95%。
...
失败 Run 仍进入可靠性分母。对 latency，已越过 Run hard deadline 或缺失合法 terminal timestamp 的 Run 以该 mode hard deadline 加 1 毫秒计入，防止快速失败或缺失时间戳改善 p95；其他失败使用真实 terminal latency。Provider transient 单独报告，同时计入 degraded 或 failure 指标；CONTENT_GAP 不计为基础设施失败，但会影响 coverage。
...
- Full Campaign 的 Provider-call 调度停止阈值为 480；Smoke 为 32。它们都不是绝对上限。
- Full 由 120 Run × 每 Run 最坏 8 次得出的代码侧绝对调用上限为 960；Smoke 为 6 × 8 = 48。若要把绝对上限降到该数学上限以下，必须把 Campaign identity 传入 ModelInvocation 的数据库 admission 并原子执行全局预算；首版不把 Campaign 逻辑耦合进被测 Worker。
- 每个 claim 按最坏 8 次调用和策略 token 上限做 in-flight reserve，最多同时保留两个。
- 每完成 10 个 Result 重新汇总 token、费用、POSSIBLY_BILLED 和失败趋势。
- 每次 admission 都检查 observed + reserved；每完成 10 个 Result 做额外趋势检查。超过估算费用阈值后禁止新提交，已出站调用照常封账。
...
- Campaign ID、commit SHA、clean-worktree 证明、API/Worker/Dispatcher image ID、OCI revision、Alembic head、manifest hash 和运行配置指纹。
...
- 错误分类优先级。
- observed/reserved 费用、Run、Smoke 32/48 与 Full 480/960 调用边界。
...
3. Candidate provenance 能证明 clean source、相同 immutable image、OCI revision、迁移 head 和安全配置指纹。
4. 6 次 Docker 小 Campaign 通过且无敏感内容、重复 Run 或悬挂调用；120-run 成本预测未超过授权阈值。
5. 120 次 Campaign 在既定调度阈值内完成，或以可解释的停止原因结束，并完成 20 个结构化人工 review。
